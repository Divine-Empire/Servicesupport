...
const [showDropdown, setShowDropdown] = useState(false);
  ...
  const handleCompanySelection = (selectedCompany) => {
    const companyData = clientData.find(item => item.companyName === selectedCompany);
...
const saveCustomCompanyName = async (companyName, state, gstNo, siteAddress) => {
  try {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const companyExists = clientData.find(
      (item) => item.companyName.toLowerCase() === formData.companyName.toLowerCase()
    );
...
if (!companyExists) {
  try {
    ...
    const bodyString =
      "sheetName=" + encodeURIComponent("CLIENT DATA").replace(/%20/g, " ") +
      "&action=insert" +
      "&rowData=" + encodeURIComponent(JSON.stringify(clientRowData));
    ...
    if (clientResult.success) {
      ...
      console.error("❌ Failed to save company data:", clientResult.error);
    }
  } catch (clientError) {
    ...
    const response = await fetch(sheet_url, {
      ...
      body: new URLSearchParams({
        ...
        columnData: JSON.stringify({
          ...
          T: fileUrlBillAttachment || "N/A", // Bill Attachment URL
    ...
    if (result.success) {
      ...
      toast({
        title: "Success",
        description: "Ticket details saved successfully" + (!companyExists ? " and new company data added" : ""),
      });
  ...
  {showCustomCompanyInput ? (
        ...
        <Button
          ...
          onClick={() => {
            if (customCompanyName.trim()) {
              handleInputChange("companyName", customCompanyName);
              setShowCustomCompanyInput(false);
              setIsAutoFilled(false);
            }
          }}
    ...
    <div className="relative">
      <div className="relative">
        <Input
          ...
          onChange={(e) => {
            ...
            if (matchedCompany) {
              handleCompanySelection(matchedCompany.companyName);
            } else {
              setIsAutoFilled(false);
            }
          }}
      ...
      {formData.companyName &&
        !clientData.find((c) => c.companyName === formData.companyName) &&
        !showCustomCompanyInput && (
          <div className="mt-2">
            <Button
              type="button"
              ...
              Save Company Data
                  ...
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <Input
                      ...
                      className={isAutoFilled ? "bg-green-50 border-green-200" : ""}
                    />
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-1">
                    <Label className="text-sm">
                      Site Address * {isAutoFilled && <span className="text-green-600 text-xs">(Auto-filled)</span>}
                    </Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label className="text-sm">
                      State * {isAutoFilled && <span className="text-green-600 text-xs">(Auto-filled)</span>}
                    </Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label className="text-sm">
                      GST NO. * {isAutoFilled && <span className="text-green-600 text-xs">(Auto-filled)</span>}
                    </Label>
                    <Input
