    ...
    try {
      ...
      if (json.success && Array.isArray(json.data)) {
        ...
        const pending = allData.filter(
          (item) => item.planned10 !== "" && item.actual10 === ""
        );
          ...
          filteredHistoryData.map((ticket, ind) => (
            <Card key={ind} className={`${ind % 2 === 0 ? "bg-blue-50/50" : "bg-white"} border-l-4 border-l-blue-500`}>
              <CardContent className="p-4 space-y-3">
                ...
                {/* Site Visit Date */}
                <div>
                  <p className="text-gray-500 font-medium text-sm">Site Visit Date</p>
                  <p className="text-blue-900">{formatDate(ticket.siteVisitDate) || "N/A"}</p>
                </div>

                {/* Verification Status */}
                <div>
                  <p className="text-gray-500 font-medium text-sm">Verification Status</p>
                  <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    ticket.verificationStatus === "yes" 
                      ? "bg-green-100 text-green-800" 
                      : "bg-yellow-100 text-yellow-800"
                  }`}>
                    {ticket.verificationStatus === "yes" ? "Verified" : "Pending"}
                  </span>
                </div>
              </CardContent>
            </Card>
